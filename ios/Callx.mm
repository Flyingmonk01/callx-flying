...
- (void)loadConfiguration {
    // Prefer persisted configuration; fallback to defaults only
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSDictionary *persisted = [defaults objectForKey:@"callx_configuration"];
    if (persisted && [persisted isKindOfClass:[NSDictionary class]]) {
        self.configuration = persisted;
        RCTLogInfo(@"Callx: Configuration loaded from NSUserDefaults");
        return;
    }
    ...
    RCTLogInfo(@"Callx: Using default configuration (no persisted config)");
}
...
RCT_EXPORT_METHOD(initialize:(NSDictionary *)config
    ...
    @try {
        ...
        if (config) {
            ...
            // Persist to NSUserDefaults for next launches
            [[NSUserDefaults standardUserDefaults] setObject:self.configuration forKey:@"callx_configuration"];
