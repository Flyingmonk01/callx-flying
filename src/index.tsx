...
class CallxManager {
  private listeners: {
    [K in keyof CallEventListeners]: ((data: any) => void)[];
  } = {
    onIncomingCall: [],
    onCallAnswered: [],
    onCallDeclined: [],
   ...
   */
  async initialize(
    config: CallxConfig | (CallxConfig & CallEventListeners)
  ): Promise<void> {
    // Initialize native module with configuration
    await Callx.initialize(config as CallxConfig);
    // If listener callbacks are present, wire them (back-compat convenience)
    const cfg = config as CallxConfig & CallEventListeners;
    if (
      cfg.onIncomingCall ||
      cfg.onCallAnswered ||
      cfg.onCallDeclined ||
      cfg.onCallEnded ||
      cfg.onCallMissed ||
      cfg.onCallAnsweredElsewhere ||
      cfg.onTokenUpdated
    ) {
      const listeners: CallEventListeners = {};
      ...
      if (cfg.onCallAnsweredElsewhere)
        listeners.onCallAnsweredElsewhere = cfg.onCallAnsweredElsewhere;
  ...
  removeAllListeners(): void {
    Object.keys(this.listeners).forEach((event) => {
      this.listeners[event as keyof CallEventListeners] = [];
    });
    this.cleanupEventListeners();
  }
   ...
   */
  private setEventListeners(listeners: CallEventListeners): void {
    ...
    if (listeners.onCallAnsweredElsewhere) {
      if (!this.listeners.onCallAnsweredElsewhere)
        this.listeners.onCallAnsweredElsewhere = [];
      this.listeners.onCallAnsweredElsewhere.push(
        listeners.onCallAnsweredElsewhere
      );
    }
   ...
   * @returns Promise that resolves when initialization is complete
   */
  async initializeWithListeners(
    config: CallxConfig & CallEventListeners
  ): Promise<void> {
    ...
    if (
      ...
      config.onCallAnswered ||
      config.onCallDeclined ||
      config.onCallEnded ||
      config.onCallMissed ||
      config.onCallAnsweredElsewhere ||
      config.onTokenUpdated
    ) {
      const listeners: CallEventListeners = {};
      if (config.onIncomingCall)
        listeners.onIncomingCall = config.onIncomingCall;
      if (config.onCallAnswered)
        listeners.onCallAnswered = config.onCallAnswered;
      if (config.onCallDeclined)
        listeners.onCallDeclined = config.onCallDeclined;
      ...
      if (config.onCallAnsweredElsewhere)
        listeners.onCallAnsweredElsewhere = config.onCallAnsweredElsewhere;
      if (config.onTokenUpdated)
        listeners.onTokenUpdated = config.onTokenUpdated;
  ...
  private attachNativeEventListeners() {
    ...
    if (this.listeners.onCallAnsweredElsewhere!.length > 0) {
      add('onCallAnsweredElsewhere', (d) =>
        this.emit('onCallAnsweredElsewhere', d)
      );
    }
  ...
  handleTokenUpdate(token: string): void {
    this.emit('onTokenUpdated', { token });
  }
...
export const initialize =
  CallxInstance.initializeWithListeners.bind(CallxInstance);
