...
object CallxStorage {
  ...
  fun saveConfiguration(context: Context, configJson: JSONObject) {
    try {
      context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        .edit()
        .putString(KEY_CONFIGURATION, configJson.toString())
        .apply()
    } catch (_: Exception) {
    }
  }
  ...
  fun getConfiguration(context: Context): JSONObject? {
    return try {
      val stored = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        .getString(KEY_CONFIGURATION, null) ?: return null
      JSONObject(stored)
    } catch (_: Exception) {
      null
    }
