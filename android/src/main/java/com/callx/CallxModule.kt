...
class CallxModule(reactContext: ReactApplicationContext) :
  ...
  init {
    ...
    // Load configuration from persisted prefs; fallback to hardcoded defaults
    val persisted = loadConfigurationFromPrefs()
    if (!persisted) {
  ...
  override fun initialize(config: ReadableMap?, promise: Promise) {
    try {
      ...
      if (config != null && (config.hasKey("triggers") || config.hasKey("fields") || config.hasKey("notification"))) {
        ...
        // Persist provided configuration for next launches
        try {
          val json = org.json.JSONObject().apply {
            // app
            put("app", org.json.JSONObject().apply {
              put("showOverLockscreen", configuration.app.showOverLockscreen)
              put("requireUnlock", configuration.app.requireUnlock)
              put("supportsVideo", configuration.app.supportsVideo)
              put("enabledLogPhoneCall", configuration.app.enabledLogPhoneCall)
            })
            // triggers
            val triggersJson = org.json.JSONObject()
            for ((key, value) in configuration.triggers) {
              triggersJson.put(key, org.json.JSONObject().apply {
            ...
            for ((key, value) in configuration.fields) {
              fieldsJson.put(key, org.json.JSONObject().apply {
                put("field", value.field)
                if (value.fallback != null) put("fallback", value.fallback) else put("fallback", org.json.JSONObject.NULL)
              })
            }
            put("fields", fieldsJson)
          }
          CallxStorage.saveConfiguration(reactApplicationContext, json)
        } catch (_: Exception) { }
      }
  ...
  override fun getConfiguration(promise: Promise) {
    try {
      ...
      appMap.putBoolean("supportsVideo", configuration.app.supportsVideo)
      appMap.putBoolean("enabledLogPhoneCall", configuration.app.enabledLogPhoneCall)
      result.putMap("app", appMap)

      // triggers
      val triggersMap = Arguments.createMap()
      for ((key, trigger) in configuration.triggers) {
        val t = Arguments.createMap()
        t.putString("field", trigger.field)
        t.putString("value", trigger.value)
        triggersMap.putMap(key, t)
      }
      result.putMap("triggers", triggersMap)

      // fields
      val fieldsMap = Arguments.createMap()
      for ((key, fieldCfg) in configuration.fields) {
        val f = Arguments.createMap()
        f.putString("field", fieldCfg.field)
        if (fieldCfg.fallback != null) {
          f.putString("fallback", fieldCfg.fallback)
        } else {
          f.putNull("fallback")
        }
        fieldsMap.putMap(key, f)
      }
  ...
  private fun loadConfigurationFromPrefs(): Boolean {
    return try {
      val json = CallxStorage.getConfiguration(reactApplicationContext) ?: return false
      // Parse app
      ...
      if (fieldsObj != null) {
        ...
        while (keys.hasNext()) {
          ...
          fields[key] = FieldConfig(f.optString("field", ""), if (f.isNull("fallback")) null else f.optString("fallback", null))
        }
      }
      configuration = CallxConfiguration(
        app = appConfig,
      ...
      android.util.Log.i(NAME, "Configuration loaded from persisted prefs")
      true
    } catch (e: Exception) {
      false
    }
  ...
  // Manifest-based configuration removed. Configuration is provided via JS initialize() and persisted.
