...
class CallxActivityHelper(private val activity: Activity) {
    ...
    private fun loadCallxConfig(): CallxAppConfig {
        // Prefer persisted configuration saved by JS initialize(); fallback to defaults
        return try {
            val json = CallxStorage.getConfiguration(activity)
            if (json != null) {
                val app = json.optJSONObject("app")
                val showOverLockscreen = app?.optBoolean("showOverLockscreen", true) ?: true
                val requireUnlock = app?.optBoolean("requireUnlock", false) ?: false
                CallxAppConfig(
            ...
            Log.w(TAG, "Failed to load persisted config: ${e.message}")
            CallxAppConfig()
        }
    }
